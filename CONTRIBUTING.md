...
├── cmp-mediaviewer/                   # The actual library code
│   ├── src/
...
1. **Clone the repository**
   ```bash
   git clone https://github.com/aryapreetam/cmp-mediaviewer.git
   cd cmp-mediaviewer
   ```
...
3. **Run tests**
   ...
   ./gradlew :cmp-mediaviewer:testDebugUnitTest  # Android unit tests
   ```
...
1. Make changes in `cmp-mediaviewer/src/commonMain/kotlin/`
2. Write tests in `cmp-mediaviewer/src/commonTest/kotlin/`
3. Run tests: `./gradlew :cmp-mediaviewer:test`
4. Check code style: `./gradlew lintRelease`

### Testing Changes in Sample App

1. Make changes in `cmp-mediaviewer/`
2. The sample app automatically uses the local library via `implementation(project(":cmp-mediaviewer"))`
...
dependencies {
    implementation("io.github.aryapreetam:cmp-mediaviewer:0.0.3")
}
```
...
1. **Update version in `cmp-mediaviewer/build.gradle.kts`**
   ```kotlin
   coordinates("io.github.aryapreetam", "cmp-mediaviewer", "0.0.4") // Bump this
   ```
...
./gradlew :cmp-mediaviewer:iosSimulatorArm64Test
./gradlew :cmp-mediaviewer:wasmJsBrowserTest
./gradlew :cmp-mediaviewer:testDebugUnitTest  # Android
```
...
1. **Add target in `cmp-mediaviewer/build.gradle.kts`**
   ```kotlin
   kotlin {
     // ...existing targets...
     tvosArm64()
...
4. **Test locally**
   ```bash
   ./gradlew :cmp-mediaviewer:tvosSimulatorArm64Test
   ```
...
```bash
# Generate locally
./gradlew :cmp-mediaviewer:dokkaGeneratePublicationHtml

# View at: cmp-mediaviewer/build/dokka/html/index.html
```
...
**Issue: "Task :cmp-mediaviewer:signKotlinMultiplatformPublication not found"**
